...
#    df_prices = pd.DataFrame({symbol: adj_close})
    ...
    for index, row in df_orders.iterrows():
        ...
        if (index in df_trades.index):
            ...
            if (order < 0 and currShare > 0): # Sell
                order = -1 * currShare
                currCASH = currCASH + (price * (-1 * order)) - commission
                currShare = 0
                price = price * (1 - impact)
                df_trades.at[index, symbol] = currShare - (-1 * order)
                df_trades.at[index, "CASH"] = currCASH# + (price * (-1 * order)) - commission
            if (order > 0 and currCASH > 0): # Buy
                order = currCASH / price
                price = price * (1 + impact)
                df_trades.at[index, symbol] = currShare + order
                df_trades.at[index, "CASH"] = -1 * (price * order) - commission
                currCASH = 0#(-1 * order) * price
                currShare = order
